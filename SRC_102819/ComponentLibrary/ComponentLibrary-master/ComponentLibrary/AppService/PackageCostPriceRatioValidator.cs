...
namespace TE.ComponentLibrary.ComponentLibrary.AppService
{
	...
	public class PackageCostPriceRatioValidator : CostPriceRatioValidator, ICostPriceRatioValidator
	{
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="PackageCostPriceRatioValidator"/> class.
		/// </summary>
		...
		public PackageCostPriceRatioValidator(ICompositeComponentRepository compositeComponentRepository,
					 ...
					 costPriceRatioData.ProjectCode != null)
			{
				...
				else
				{
					return result;
					 ...
					 costPriceRatioData.ProjectCode == null)
			{
				return new Tuple<bool, string>(false, "CPR cannot be defined at package level3.");
			}
			else
			{
				return new Tuple<bool, string>(false, "Invalid combination of package level1, level2, code and project code.");
			}
		}

		private async Task<Tuple<bool, string>> ValidateLevels(string level1, string level2)
		{
			var dependencyDefinition = await _dependencyDefinitionRepository.GetDependencyDefinition("packageClassifications");
			if (
				dependencyDefinition.Blocks.Select(b => b.Data[0])
					.Distinct()
					.All(x => !string.Equals(x, level1, StringComparison.InvariantCultureIgnoreCase)))
			{
				return new Tuple<bool, string>(false, $"Invalid package level1 value.");
			}
			if (level2 != null &&
			...
			{
				return new Tuple<bool, string>(false, $"Invalid combination of package level1 and level2 values.");
			}
		...
		{
			var count = await _compositeComponentRepository.Count("package",
				new Dictionary<string, Tuple<string, object>>
				{
					{"package_code", new Tuple<string, object>("Regex", $"((?i){code}(?-i))")}
				});
			return count == 0
				? new Tuple<bool, string>(false, $"{code} is not valid. Please enter a valid package code.")
