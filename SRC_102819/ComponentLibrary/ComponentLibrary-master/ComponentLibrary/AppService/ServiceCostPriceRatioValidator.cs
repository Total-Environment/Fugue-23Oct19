...
namespace TE.ComponentLibrary.ComponentLibrary.AppService
{
	...
	public class ServiceCostPriceRatioValidator : CostPriceRatioValidator, ICostPriceRatioValidator
	{
		private readonly IServiceRepository _serviceRepository;
		private readonly IDependencyDefinitionRepository _dependencyDefinitionRepository;

		/// <summary>
		/// Initializes a new instance of the <see cref="ServiceCostPriceRatioValidator"/> class.
		/// </summary>
		/// <param name="serviceRepository">The service repository.</param>
		/// <param name="projectRepository">The project repository.</param>
		/// <param name="dependencyDefinitionRepository">The dependency definition repository.</param>
		public ServiceCostPriceRatioValidator(IServiceRepository serviceRepository, IProjectRepository projectRepository,
			IDependencyDefinitionRepository dependencyDefinitionRepository) : base(projectRepository)
		{
					 ...
					 costPriceRatioData.ProjectCode != null)
			{
				...
				else
				{
					return result;
					 ...
					 costPriceRatioData.ProjectCode == null)
			{
				return new Tuple<bool, string>(false, "CPR cannot be defined at service level3.");
			}
			else
			{
				return new Tuple<bool, string>(false, "Invalid combination of service level1, level2, code and project code.");
			}
		}

		private async Task<Tuple<bool, string>> ValidateLevels(string level1, string level2)
		{
			var dependencyDefinition = await _dependencyDefinitionRepository.GetDependencyDefinition("serviceClassifications");
			if (
				dependencyDefinition.Blocks.Select(b => b.Data[0])
					.Distinct()
					.All(x => !string.Equals(x, level1, StringComparison.InvariantCultureIgnoreCase)))
			{
				return new Tuple<bool, string>(false, $"Invalid service level1 value.");
			}
			if (level2 != null &&
				dependencyDefinition.Blocks.Where(
						b => string.Equals(b.Data[0], level1, StringComparison.InvariantCultureIgnoreCase))
					.Select(b => b.Data[1])
					.Distinct()
					.All(x => !string.Equals(x, level2, StringComparison.InvariantCultureIgnoreCase)))
			{
				return new Tuple<bool, string>(false, $"Invalid combination of service level1 and level2 values.");
			}
		...
		{
			var count = await _serviceRepository.Count(
				new Dictionary<string, Tuple<string, object>>
				{
					{"service_code", new Tuple<string, object>("Regex", $"((?i){code}(?-i))")}
				});
			return count == 0
				? new Tuple<bool, string>(false, $"{code} is not valid. Please enter a valid service code.")
